...
export class InicioComponent implements AfterViewInit {
	...
	private capaWMSActual: L.TileLayer.WMS | null = null;
	private capasWMS = [
		{ nombre: 'Bosque Yungas', layer: 'bosque_yungas' },
		{ nombre: 'Bosque Tucumano', layer: 'bosque_tucumano' },
		{ nombre: 'Bosque Pantanal', layer: 'bosque_pantanal' },
		{ nombre: 'Bosque de Llanuras', layer: 'bosque_llanuras' },
		{ nombre: 'Bosque Interandino', layer: 'bosque_interandino' },
		{ nombre: 'Bosque Chiquitano', layer: 'bosque_chiquitano' },
		{ nombre: 'Bosque Chaqueño', layer: 'bosque_chaqueno' },
		{ nombre: 'Bosque Andino', layer: 'bosque_andino' },
		{ nombre: 'Bosque Amazonico', layer: 'bosque_amazonico' },
	];
	...
	private iniciarMapa(): void {
		...
		this.capasWMS.forEach(capa => {
			this.agregarCapaWMS(capa.nombre, capa.layer);
		});
	}
	verDetalles(data: string) {
		...
		if (this.bosqueSeleccionado) {
			console.log(data)
			this.agregarCapaWMS(this.bosqueSeleccionado.layer, this.bosqueSeleccionado.layer);
	...
	private capasWMSActivas: L.Layer[] = [];

	private agregarCapaWMS(nombre: string, layer: string): void {
		const capa = L.tileLayer.wms('https://geoserver.bits.bo/geoserver/afd/wms', {
			layers: layer,
			format: 'image/png',
			transparent: true
		}).addTo(this.map);

		this.capasWMSActivas.push(capa);
	}
	private limpiarMapa(): void {
		this.capasWMSActivas.forEach(capa => this.map.removeLayer(capa));
		this.capasWMSActivas = [];
	}
}
