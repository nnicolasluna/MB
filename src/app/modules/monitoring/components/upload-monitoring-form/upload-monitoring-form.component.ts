...
@Component({
	selector: 'app-upload-monitoring-form',
	imports: [
		FormsModule,
		ReactiveFormsModule,
		LoaderComponent,
	...
	templateUrl: './upload-monitoring-form.component.html',
	styleUrl: './upload-monitoring-form.component.scss',
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UploadMonitoringFormComponent extends BaseFormComponent<any> {
	...
	constructor() {
		...
		this._service = this._config.data.service;
	}
	...
	override buildForm(): void {
		...
		if (this.currentItem().coverageDate) {
			const utcDate = dayjs(this.currentItem().coverageDate).utc();
			contentDate = new Date();
			contentDate.setUTCFullYear(utcDate.year(), utcDate.month(), utcDate.date());
		}
	...
	public saveForm(): void {
		...
		this._form.disable();
		this.isLoading.set(true);
		if (data.has('file')) this.uploadStatus.set(MapUploadStatus.UPLOADING);
	...
	public columns = this._config.data.columns ?? [];
}
