...
export class CalendarGanttComponent implements OnInit {
  ...
  getTaskPosition(task: GanttTask) {
    ...
    const startMonthIndex = this.timeScale.findIndex(date =>
      date.getFullYear() === task.startDate.getFullYear() &&
      date.getMonth() === task.startDate.getMonth()
    );

    const endMonthIndex = this.timeScale.findIndex(date =>
      date.getFullYear() === task.endDate.getFullYear() &&
      date.getMonth() === task.endDate.getMonth()
    );
  ...
  mapToGanttTasks(actividad: any): GanttTask[] {
    ...
    return actividad.Tarea.map((tarea: any) => {
      const fechas = tarea.FechaProgramada
        .map((f: any) => {
